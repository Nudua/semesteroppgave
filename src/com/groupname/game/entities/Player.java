...
public class Player extends Actor {
    ...
    public void update() {
        ...
        } else if(inputManager.isDown(KeyboardInput.Defaults.SHOOT_LEFT)) {
            if(!bullet1.isAlive()) {
                bullet1.fire(new Vector2D(position.getX() + sprite.getWidth() / 2, position.getY()), Direction.Left);
            }
        } else if(inputManager.isDown(KeyboardInput.Defaults.SHOOT_DOWN)) {
            if(!bullet1.isAlive()) {
                bullet1.fire(new Vector2D(position.getX() + sprite.getWidth() / 2, position.getY()), Direction.Down);
            }
        } else if(inputManager.isDown(KeyboardInput.Defaults.SHOOT_UP)) {
            if(!bullet1.isAlive()) {
